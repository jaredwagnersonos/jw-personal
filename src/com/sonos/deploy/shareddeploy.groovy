    def call(body) {
        ...
        node {
          stage('Deploy'){
                    ...
                    echo """curl -k --user ${env.TOWER_ACCESS_USR}:${env.TOWER_ACCESS_PSW} 'https://${env.TOWER_SERVER}/api/v1/job_templates/${env.TOWER_JOB_TPLT_ID}/launch/' -X POST -d '{ "extra_vars": { "env": "${env.DEPLOY_ENV}", "version": "0.0.4-MSSL-SNAPSHOT", "deployment_iteration": 0, "aws_region": "us-east-1" }, "vault_password": "${env.TOWER_VAULT_PWD}" }' -H 'Content-Type:application/json' > tower_launch_output"""
                        ...
                        script {
                            ...
                            while(status != 'successful' && status != 'failed') {
                                ...
                                sh """curl -k --user ${env.TOWER_ACCESS_USR}:${env.TOWER_ACCESS_PSW} \
                                      \'https://${env.TOWER_SERVER}/api/v1/jobs/${towerJobId}/\' \
                                      > tower_output"""
